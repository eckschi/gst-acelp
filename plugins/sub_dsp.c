...
void Autocorr(Word16 x[], Word16 p, Word16 r_h[], Word16 r_l[], tetra_op_t* top)
{
	...
	do 
	{
		...
		{
			for (i = 0; i < L_window; i++)
				y[i] = shr(y[i], (Word16)2, top);
		}
	
	}while (top->Overflow != 0);
	
	
	/* Normalization of r[0] */
	
	norm = norm_l(sum);
	sum  = L_shl(sum, norm, top);
	sum  = L_shr(sum, (Word16)1, top);		/* For special double format */
	L_extract(sum, &r_h[0], &r_l[0], top);
	...
	for (i = 1; i <= p; i++)
	{
		...
		for (j = 0; j < L_window - i; j++)
			sum = L_mac0(sum, y[j], y[j + i], top);
		
		sum = L_shr(sum, (Word16)1, top);		/* Special double format */
		sum = L_shl(sum, norm, top);
		L_extract(sum, &r_h[i], &r_l[i], top);
	}
	return;
}
...
void Az_Lsp(Word16 a[], Word16 lsp[], Word16 old_lsp[], tetra_op_t* top)
{
	...
	for (i = 0; i < nc; i++)
	{
		t0 = Load_sh(a[i + 1], (Word16)15, top);	/* a[i + 1]  in Q27 */
		t0 = add_sh(t0, a[pp - i], (Word16)15, top);	/* +a[pp - i] in Q27 */
		t0 = sub_sh16(t0, f1[i], top);			/* -f1[i]  in Q27 */
		f1[i + 1] = extract_h(t0);		/* f1[i + 1] = a[i + 1] + a[pp - i] - f1[i] */
		/* result in Q11  */
		
		t0 = Load_sh(a[i + 1], (Word16)15, top);	/* a[i + 1]  in Q27   */
		t0 = sub_sh(t0, a[pp - i], (Word16)15, top);	/* -a[pp - i] in Q27 */
		t0 = add_sh16(t0, f2[i], top);			/* +f2[i] in Q27  */
	...
	while ((nf < pp) && (j < grid_points))
	{
		...
		if (L_mult0(ylow, yhigh) <=(Word32)0)
		{
			...
			for (i = 0; i < 4; i++)
			{
				t0   = Load_sh(xlow, (Word16)15, top);	/* xmid = 0.5* (xlow + xhigh) */
				t0   = add_sh(t0, xhigh, (Word16)15, top);
				xmid = extract_h(t0);
				
				ymid = Chebps(xmid, coef, nc, top);
			...
			*-------------------------------------------------------------*/
			...
			x   = sub(xhigh, xlow, top);
			y   = sub(yhigh, ylow, top);
			...
			else
			{
				...
				exp = norm_s(y);
				y   = shl(y, exp, top);
				y   = div_s((Word16)16383, y, top);
				t0  = L_mult0(x, y);
				t0  = L_shr(t0, sub((Word16)19, exp, top), top);
				y    = extract_l(t0);		/* y = (xhigh - xlow)/ (yhigh - ylow) in Q10 */
				
				if (sign < 0)
					y = negate(y);
				
				t0   = Load_sh(xlow, (Word16)10, top);	/* xint = xlow - ylow*y */
				t0   = L_msu0(t0, ylow, y, top);
				xint = store_hi(t0, (Word16)6, top);
			}
...
void Back_Fil(Word16 x[], Word16 h[], Word16 y[], Word16 L, tetra_op_t* top)
{
	...
	for (i = 0; i < L; i++)
	{
		s = 0;
		for (j = i; j <  L; j++)
			s = L_mac0(s, x[j], h[j - i], top);
		
		y32[i] = s;
		
		s = L_abs(s);
		if (L_sub(s, max, top) > 0)
			max = s;
	}
	...
	j = norm_l(max);
	if (sub(j, (Word16)16, top) > 0)
		j = 16;
	
	j = sub((Word16)18, j, top);
	
	for (i = 0; i < L; i++)
		y[i] = extract_l(L_shr(y32[i], j, top));
...
Word16 Chebps(Word16 x, Word16 f[], Word16 n, tetra_op_t* top)
{
	...
	t0 = Load_sh(x, (Word16)10, top);		/* 2*x in Q24          */
	t0 = add_sh(t0, f[1], (Word16)13, top);	/* + f[1] in Q24       */
	L_extract(t0, &b1_h, &b1_l, top);		/* b1 = 2*x + f[1]     */
	
	for (i = 2; i < n; i++)
	{
		t0 = mpy_mix(b1_h, b1_l, x, top);		/* t0 = x*b1                  */
		t0 = L_shl(t0, (Word16)1, top);			/* t0 = 2.0*x*b1              */
		t0 = sub_sh(t0, b2_l, (Word16)0, top);	/* t0 = 2.0*x*b1 - b2         */
		t0 = sub_sh(t0, b2_h, (Word16)15, top);
		t0 = add_sh(t0, f[i], (Word16)13, top);
		/* t0 = 2.0*x*b1 - b2 + f[i]; */
		
		
		L_extract(t0, &b0_h, &b0_l, top);
		/* b0 = 2.0*x*b1 - b2 + f[i]; */
		b2_l = b1_l;
		/* b2 = b1; */
		b2_h = b1_h;
	...
	t0 = mpy_mix(b1_h, b1_l, x, top);
	/* t0    = x*b1;     */
	t0 = sub_sh(t0, b2_l, (Word16)0, top);
	/* t0   -= b2;       */
	t0 = sub_sh(t0, b2_h, (Word16)15, top);
	t0 = add_sh(t0, f[n], (Word16)12, top);
	/* t0   += 0.5*f[n]; */
	
	t0 = L_shl(t0, (Word16)6, top);			/* Q24 to Q30 with saturation */
...
void Convolve(Word16 x[], Word16 h[], Word16 y[], Word16 L, tetra_op_t* top)
{
	...
	for (n = 0; n < L; n++)
	{
		s = 0;
		for (i = 0; i <= n; i++)
			s = L_mac0(s, x[i], h[n - i], top);
		y[n] = store_hi(s, (Word16)4, top);		/* h is in Q12 */
	}
...
void Fac_Pond(Word16 gamma, Word16 fac[], tetra_op_t* top)
{
	...
	fac[0] = gamma;
	for (i = 1; i < pp; i++)
		fac[i] = acelp_round(L_mult(fac[i - 1], gamma, top), top);
...
void Get_Lsp_Pol(Word16 *lsp, Word32 *f, tetra_op_t* top)
{
	...
	*f = Load_sh((Word16)4096, (Word16)12, top);
	/* f[0] = 1.0;           in Q24  */
	f++;
	*f = 0;
	*f = sub_sh(*f, *lsp, (Word16)10, top);
	/* f[1] = -2.0 * lsp[0]; in Q24  */
	f++;
	...
	for (i = 2; i <= 5; i++)
	{
		*f = f[-2];
		...
		for (j = 1; j < i; j++, f--)
		{
			L_extract(f[-1] , &hi, &lo, top);
			t0 = mpy_mix(hi, lo, *lsp, top);		/* t0 = f[-1] * lsp    */
			t0 = L_shl(t0, (Word16)1, top);
			*f = L_add(*f, f[-2], top);			/* *f += f[-2]         */
			*f = L_sub(*f, t0, top);			/* *f -= t0            */
		}
		*f   = sub_sh(*f, *lsp, (Word16)10, top);	/* *f -= lsp << 10       */
...
void Int_Lpc4(Word16 lsp_old[], Word16 lsp_new[], Word16 a[], tetra_op_t* top)
{
	...
	for (j = 0; j < 33; j += 11)
	{
		for (i = 0; i < pp; i++)
		{
			t0 = L_mult(lsp_old[i], fac_old, top);
			t0 = L_mac(t0, lsp_new[i], fac_new, top);
			lsp[i] = extract_h(t0);
		}
		Lsp_Az(lsp, &a[j], top);
		
		fac_old = sub(fac_old, (Word16)8192, top);
		fac_new = add(fac_new, (Word16)8192, top);
	}
	Lsp_Az(lsp_new, &a[33], top);
...
void Lag_Window(Word16 p, Word16 r_h[], Word16 r_l[], tetra_op_t* top)
{
	...
	for (i = 1; i <= p; i++)
	{
		x  = mpy_32(r_h[i], r_l[i], lag_h[i - 1], lag_l[i - 1], top);
		L_extract(x, &r_h[i], &r_l[i], top);
	}
	return;
}
...
void Levin_32(Word16 Rh[], Word16 Rl[], Word16 A[], tetra_op_t* top)
{
	...
	Word16 alp_h, alp_l, alp_e;
	/* Prediction gain; hi lo and exponant       */
	...
	t1  = L_comp(Rh[1], Rl[1], top);			/* R[1] in Q30      */
	t2  = L_abs(t1);					/* abs R[1]         */
	t0  = div_32(t2, Rh[0], Rl[0], top);		/* R[1]/R[0] in Q30 */
	if (t1 > 0)
		t0= L_negate(t0);				/* -R[1]/R[0]       */
	L_extract(t0, &Kh, &Kl, top);			/* K in DPF         */
	t0 = L_shr(t0, (Word16)4, top);			/* A[1] in Q26      */
	L_extract(t0, &Ah[1], &Al[1], top);		/* A[1] in DPF      */
	
	
	/*  Alpha = R[0] * (1 - K**2) */
	
	t0 = mpy_32(Kh , Kl, Kh, Kl, top);		/* K*K      in Q30 */
	t0 = L_abs(t0);						/* Some case <0 !! */
	t0 = L_sub((Word32)0x3fffffff, t0, top);	/* 1 - K*K  in Q30 */
	L_extract(t0, &hi, &lo, top);			/* DPF format      */
	t0 = mpy_32(Rh[0] , Rl[0], hi, lo, top);	/* Alpha in Q30    */
	
	/* Normalize Alpha */
	
	t0 = norm_v(t0, (Word16)12, &i, top);
	t0 = L_shr(t0, (Word16)1, top);
	L_extract(t0, &alp_h, &alp_l, top);		/* DPF format    */
	...
	for (i= 2; i <= pp; i++)
	{
		...
		t0 = 0;
		for (j = 1; j < i; j++)
			t0 = L_add(t0, mpy_32(Rh[j], Rl[j], Ah[i - j], Al[i - j], top), top);
		
		t0 = L_shl(t0, (Word16)4, top);			/* result in Q26->convert to Q30 */
		/* No overflow possible          */
		t1 = L_comp(Rh[i], Rl[i], top);
		t0 = L_add(t0, t1, top);				/* add R[i] in Q30               */
		
		/* K = -t0 / Alpha */
		
		t1 = L_abs(t0);
		t2 = div_32(t1, alp_h, alp_l, top);		/* abs(t0)/Alpha                 */
		if (t0 > 0)
			t2= L_negate(t2);		/* K =-t0/Alpha                  */
		t2 = L_shl(t2, alp_e, top);
		/* denormalize; compare to Alpha */
		L_extract(t2, &Kh, &Kl, top);			/* K in DPF                      */
		...
		*-------------------------------------------*/
		...
		for (j = 1; j < i; j++)
		{
			t0 = mpy_32(Kh, Kl, Ah[i - j], Al[i - j], top);
			t0 = add_sh(t0, Ah[j], (Word16)15, top);
			t0 = add_sh(t0, Al[j], (Word16)0, top);
			L_extract(t0, &Anh[j], &Anl[j], top);
		}
		t2 = L_shr(t2, (Word16)4, top);		/* t2 = K in Q30->convert to Q26 */
		L_extract(t2, &Anh[i], &Anl[i], top);	/* An[i] in Q26                  */
		...
		t0 = L_sub((Word32)0x3fffffff, t0, top);	/* 1 - K*K  in Q30 */
		L_extract(t0, &hi, &lo, top);			/* DPF format      */
		t0 = mpy_32(alp_h , alp_l, hi, lo, top);	/* Alpha in Q30    */
		
		/* Normalize Alpha */
		
		t0 = norm_v(t0, (Word16)12, &j, top);
		t0 = L_shr(t0, (Word16)1, top);
		L_extract(t0, &alp_h, &alp_l, top);		/* DPF format    */
	...
	for (i = 1; i <= pp; i++)
	{
		t0   = L_comp(Ah[i], Al[i], top);
		t0   = add_sh(t0, (Word16)1, (Word16)13, top);	/* rounding */
		top->old_A[i] = A[i] = store_hi(t0, (Word16)2, top);
	}
...
Word32 Lpc_Gain(Word16 a[], tetra_op_t* top)
{
	...
	for (i = 1; i < llg; i++)
		h[i] = 0;
	Syn_Filt(a, h, h, llg, &h[1], (Word16)0, top);
	
	/* Compute the energy of the impulse response */
	
	ener = 0;
	for (i = 0; i < llg; i++)
		ener = L_mac0(ener, h[i], h[i], top);
...
void Lsp_Az(Word16 lsp[], Word16 a[], tetra_op_t* top)
{
	...
	Get_Lsp_Pol(&lsp[0], f1, top);
	Get_Lsp_Pol(&lsp[1], f2, top);
	
	for (i = 5; i > 0; i--)
	{
		f1[i] = L_add(f1[i], f1[i - 1], top);
		/* f1[i] += f1[i - 1]; */
		f2[i] = L_sub(f2[i], f2[i - 1], top);
		/* f2[i] -= f2[i - 1]; */
	}
	...
	{
		t0   = L_add(f1[i], f2[i], top);			/* f1[i] + f2[i] */
		a[i] = extract_l(L_shr_r(t0, (Word16)13, top));	/*from Q24 to Q12 and * 0.5*/
		t0   = L_sub(f1[i], f2[i], top);			/* f1[i] - f2[i] */
		a[j] = extract_l(L_shr_r(t0, (Word16)13, top));	/*from Q24 to Q12 and * 0.5*/
	}
...
void Pond_Ai(Word16 a[], Word16 fac[], Word16 a_exp[], tetra_op_t* top)
{
	...
	a_exp[0] = a[0];
	for (i = 1; i <= pp; i++)
		a_exp[i] = acelp_round(L_mult(a[i], fac[i - 1], top), top);
...
void Residu(Word16 a[], Word16 x[], Word16 y[], Word16 lg, tetra_op_t* top)
{
	...
	for (i = 0; i < lg; i++)
	{
		s = Load_sh(x[i], (Word16)12, top);
		for (j = 1; j <= pp; j++)
			s = L_mac0(s, a[j], x[i - j], top);
		
		s = add_sh(s, (Word16)1, (Word16)11, top);		/* Rounding */
		s = L_shl(s, (Word16)4, top);				/* Saturation */
		y[i] = extract_h(s);
	}
	return;
}
              ...
              Word16 update, tetra_op_t* top)
{
	...
	for (i = 0; i < lg; i++)
	{
		s = Load_sh(x[i], (Word16)12, top);				/* a[] is in Q12 */
		for (j = 1; j <= pp; j++)
			s = L_msu0(s, a[j], yy[-j], top);
		
		s     = add_sh(s, (Word16)1, (Word16)11, top);		/* Rounding */
		*yy++ = extract_h(L_shl(s, (Word16)4, top));
	}
