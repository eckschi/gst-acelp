...
void Bits2prm_Tetra(Word16 *bits, Word16 prm[], tetra_op_t* top)
{
	...
	*prm++ = *bits++;     /* read BFI */
	...
	for (i = 0; i < PRM_NO; i++)
	{
		prm[i] = bin2int(bitno[i], bits, top);
		bits  += bitno[i];
	}
}
...
void Bits2prm_Tetra_8(unsigned char *bits, unsigned char bfi, Word16 prm[], tetra_op_t* top)
{
	...
	static const unsigned int bitno[PRM_NO] = 
	{
		8, 9, 9,            /* split VQ LSP  */
		8, 14, 1, 1, 6,     /* subframe 1    */
		5, 14, 1, 1, 6,     /* subframe 2    */
	...
	*prm++ = bfi;     /* read BFI */
	...
	for (i = 0; i < PRM_NO; i++)
	{
		prm[i] = bin2int_8(bitno[i], bits, top);
		bits  += bitno[i];
	}
}
...
void Cal_Rr2(Word16 h[], Word16 *rr, tetra_op_t* top)
{
	...
	s = 0;
	for (i = 0; i < L; i++)
		s = L_mac0(s, h[i], h[i], top);
	
	k = norm_l(s);
	k = shr(sub(k, (Word16)1, top), (Word16)1, top);
...
void Clsp_334(Word16 *lsp, Word16 *lsp_q, Word16 *indice, tetra_op_t* top)
{
	...
	{
		temp  = sub(lsp[0], *p_dico++, top);
		dist = L_mult0(temp, temp);
		for (j = 1; j < 3; j++)
		{
			temp  = sub(lsp[j], *p_dico++, top);
			dist  = L_mac0(dist, temp, temp, top);
		}
		if (L_sub(dist, min, top) < 0)
		{
			min = dist;
			ind = i; 
		}
	...
	temp = sub(temp, lsp_q[2], top);
	temp = add(temp, lsp_q[3], top);    /* temp = 0.028 - (lsp_q[2] - lsp_q[3])      */
	if (temp > 0)
	{
		temp = shr(temp, (Word16)1, top);
		lsp_q[2] = add(lsp_q[2], temp, top);
		lsp_q[3] = sub(lsp_q[3], temp, top);
	}
	/* Minimum distance between lsp_q[5] and lsp_q[6] */
	
	temp = 1245;                   /* 1245= 0.038 in Q15 = 50hz around 1600hz */
	temp = sub(temp, lsp_q[5], top);
	temp = add(temp, lsp_q[6], top);    /* temp = 0.038 - (lsp_q[5] - lsp_q[6])      */
	if (temp > 0)
	{
		temp = shr(temp, (Word16)1, top);
		lsp_q[5] = add(lsp_q[5], temp, top);
		lsp_q[6] = sub(lsp_q[6], temp, top);
	}
	...
	for (i = 0; i < 9; i++)
	{
		if (sub(lsp_q[i], lsp_q[i + 1], top) <= 0)
		{
			temp = 1;
		}
	}
	...
	{
		for (i = 0; i < 10; i++)
			lsp_q[i] = top->lsp_old[i];
	}
	
	else
	{
		for (i = 0; i < 10; i++)
			top->lsp_old[i] = lsp_q[i];
	}
	return;
}
		  ...
		  tetra_op_t* top)
{
	...
	L_tmp = Lpc_Gain(A, top);
	
	exp_lpc  = norm_l(L_tmp);
	ener_lpc = extract_h(L_shl(L_tmp, exp_lpc, top));
	...
	for (i = 0; i < L_subfr; i++)
		L_tmp = L_mac0(L_tmp, prd_lt[i], prd_lt[i], top);
	
	exp_plt  = norm_l(L_tmp);
	ener_plt = extract_h(L_shl(L_tmp, exp_plt, top));
	
	/* ener_plt = Log2(ener_plt * ener_lpc) */
	
	L_tmp = L_mult0(ener_plt, ener_lpc);
	exp_plt = add(exp_plt, exp_lpc, top);
	
	Log2(L_tmp, &exp, &frac, top);
	
	L_tmp = Load_sh16(exp, top);
	L_tmp = add_sh(L_tmp, frac, (Word16)1, top);	/* Log2(ener_plt*ener_lpc) in Q16*/
	L_tmp = sub_sh16(L_tmp, exp_plt, top);		/* subtract exponant of ener_plt */
	
	/* Input on 15 bits */
	L_tmp = add_sh(L_tmp, (Word16)1710, (Word16)8, top); /* +6.68 in Q16 
	for other scaling    */
	
	L_tmp = L_shr(L_tmp, (Word16)8, top);		/* Convert from Q16 to Q8        */
	...
	L_tmp = 0;
	for (i = 0; i < L_subfr; i++)
		L_tmp = L_mac0(L_tmp, code[i], code[i], top);
	...
	L_tmp = Load_sh16(exp, top);
	L_tmp = add_sh(L_tmp, frac, (Word16)1, top);	/* Log2(ener_plt*ener_lpc) in Q16*/
	L_tmp = sub_sh16(L_tmp, exp_lpc, top);		/* subtract exponant of ener_lpc */
	
	/* Input on 15 bits */
	L_tmp = sub_sh(L_tmp, (Word16)4434, (Word16)8, top); /*-17.32 in Q16 
	for other scaling    */
	...
	if (bfi != 0)
	{
		top->last_ener_pit = sub(top->last_ener_pit, (Word16)128, top);	/* -0.5 in Q8 */
		if (top->last_ener_pit < 0)
			top->last_ener_pit = 0;
		
		top->last_ener_cod = sub(top->last_ener_cod, (Word16)128, top);	/* -0.5 in Q8 */
		if (top->last_ener_cod < 0)
			top->last_ener_cod = 0;
	}
	else
	{
	 ...
	 *-----------------------------------------------------------------*/
		...
		L_tmp = Load_sh(top->last_ener_pit, (Word16)8, top);	/* .5 last_ener_pit in Q9  */
		L_tmp = add_sh(L_tmp, top->last_ener_cod, (Word16)7, top); /*+.25 last_ener_code 	in Q9    */
		L_tmp = sub_sh(L_tmp, (Word16)768, (Word16)9, top); /* -3.0 in Q9          */
		if (L_tmp < 0)
			L_tmp = 0;
		pred_pit = store_hi(L_tmp, (Word16)7, top);	/* result in Q8 		   */
		...
		*-----------------------------------------------------------------*/
		...
		L_tmp = Load_sh(top->last_ener_cod, (Word16)8, top);	/* .5 last_ener_cod in Q9  */
		L_tmp = add_sh(L_tmp, top->last_ener_pit, (Word16)7, top); /*+.25 last_ener_pit in Q9    */
		L_tmp = sub_sh(L_tmp, (Word16)768, (Word16)9, top); /* -3.0 in Q9          */
		if (L_tmp < 0)
			L_tmp = 0;
		pred_cod = store_hi(L_tmp, (Word16)7, top);	/* result in Q8		   */
		
		/*-----------------------------------------------------------------*
		* Read quantized values.                                          *
		*-----------------------------------------------------------------*/
		
		j = shl(index, (Word16)1, top);
		top->last_ener_pit = add(t_qua_ener[j],   pred_pit, top);
		top->last_ener_cod = add(t_qua_ener[j + 1], pred_cod, top);
		
		/* Limit energies->for transmission errors */
		
		if (sub(top->last_ener_pit, (Word16)6912, top) > 0)
			top->last_ener_pit = 6912; 
		/* 6912 = 27 in Q8 */
		if (sub(top->last_ener_cod, (Word16)6400, top) > 0)
			top->last_ener_cod = 6400; 
		/* 6400 = 25 in Q8 */
	}
	...
	*---------------------------------------------------*/
	...
	L_tmp = Load_sh(top->last_ener_pit, (Word16)6, top);	 /* last_ener_pit/2 in Q15 */
	L_tmp = sub_sh(L_tmp, ener_plt, (Word16)6, top);	 /* - ener_plt/2    in Q15 */
	L_tmp = add_sh(L_tmp, (Word16)12, (Word16)15, top); /* to have gain in Q12    */
	L_extract(L_tmp, &exp, &frac, top);
	L_tmp = pow2(exp, frac, top);
	if (L_sub(L_tmp, (Word16)4915, top) > 0)
		L_tmp = 4915; /* 4915 = 1.2 in Q12   */
	...
	*---------------------------------------------------*/
	...
	L_tmp = Load_sh(top->last_ener_cod, (Word16)6, top);	/* last_ener_cod/2 in Q15 */
	L_tmp = sub_sh(L_tmp, ener_c, (Word16)6, top);	/* - ener_c/2      in Q15 */
	L_extract(L_tmp, &exp, &frac, top);
	L_tmp = pow2(exp, frac, top);
			...
			tetra_op_t* top)
{
	...
	for (i = 0; i < 60; i += 2)
	{
		j = add(dn[i], dn[i + 1], top);
		if (sub(j, max0, top) > 0)
			max0 = j;
		else if (sub(j, min0, top) < 0)
			min0 = j;
	}
	max0 = shr(max0, (Word16)1, top);
	min0 = shr(min0, (Word16)1, top);
	
	
	/* Multiply max0 and min0 by gain_i0 in Q11 */
	
	max0 = store_hi(L_mult0(max0, (Word16)Q11_gain_i0), (Word16)5, top);
	min0 = store_hi(L_mult0(min0, (Word16)Q11_gain_i0), (Word16)5, top);
	...
	for (i = 2; i < 60; i += 8)
	{
		...
		if (sub(j, max1, top) > 0)
			max1 = j;
		else if (sub(j, min1, top) < 0)
			min1 = j;
	}
	max1 = shr(max1, (Word16)1, top);
	min1 = shr(min1, (Word16)1, top);
	...
	for (i = 4; i < 60; i += 8)
	{
		...
		if (sub(j, max2, top) > 0)
			max2 = j;
		else if (sub(j, min2, top) < 0)
			min2 = j;
	}
	max2 = shr(max2, (Word16)1, top);
	min2 = shr(min2, (Word16)1, top);
	...
	*----------------------------------------------------------*/
	...
	max0 = sub(max0, min1, top);
	max2 = add(max0, max2, top);
	max1 = sub(max1, min0, top);
	j    = sub(max1, min2, top);
	
	if (sub(max0, max1, top) > 0)
		max1 = max0;
	if (sub(j, max2, top)    > 0)
		max2 = j;
	
	/* Set thresholds */
	seuil1 = mult(max1, (Word16)threshold1, top);
	seuil2 = mult(max2, (Word16)threshold2, top);
	...
	{
		ps0  = store_hi(L_mult0((Word16)Q11_gain_i0, dn[2*ii0]), (Word16)5, top);
		ps0a = store_hi(L_mult0((Word16)Q11_gain_i0, dn[2*ii0 + 1]), (Word16)5, top);
		alp0_32 = Load_sh(rr[ii0][ii0], (Word16)14, top); 
		
		for (ii1 = 1; ii1 < 30; ii1 += 4)
		{
			ps1  = sub(ps0, dn[2*ii1], top);
			ps1a = sub(ps0a, dn[2*ii1 + 1], top);
			
			/* alp1 = alp0*2 + rr[ii1][ii1] - 2.0*gain_i0*rr[ii0][ii1]  */
			/* The result is divided by 8 to avoid overflow.            */
			L_tmp = add_sh(alp0_32, rr[ii1][ii1], (Word16)13, top); 
			
			L_tmp = L_msu0(L_tmp, (Word16)Q14_gain_i0, rr[ii0][ii1], top);
			alp1  = extract_h(L_tmp);
			alp1_32 = Load_sh(alp1, (Word16)15, top);
			
			/* If(abs((ps1 + ps1a)/2) > seuil1 */
			
			L_tmp = Load_sh(ps1, (Word16)15, top);
			L_tmp = L_abs(add_sh(L_tmp, ps1a, (Word16)15, top));
			L_tmp = sub_sh16(L_tmp, seuil1, top);
			if (L_tmp > 0)
			{
				for (ii2 = 2; ii2 < 31; ii2 += 4)
				{
					ps2  = add(ps1, dn[2*ii2], top);
					ps2a = add(ps1a, dn[2*ii2 + 1], top);
					
					/* alp2 = alp1 + rr[ii2][ii2] + 2.0* (gain_i0*rr[ii0][ii2] - rr[ii1][ii2]) */
					/* The result is divided by 16 to avoid overflow.                 */
					
					L_tmp = add_sh(alp1_32, rr[ii2][ii2], (Word16)12, top);  
					
					L_tmp = L_mac0(L_tmp, (Word16)Q13_gain_i0, rr[ii0][ii2], top);
					L_tmp = sub_sh(L_tmp, rr[ii1][ii2] , (Word16)13, top);
					alp2  = extract_h(L_tmp);
					alp2_32 = Load_sh16(alp2, top);
					
					/* If(abs((ps2 + ps2a)/2) > seuil2 */
					
					L_tmp = Load_sh(ps2, (Word16)15, top);
					L_tmp = L_abs(add_sh(L_tmp, ps2a, (Word16)15, top));
					L_tmp = sub_sh16(L_tmp, seuil2, top);
					if (L_tmp > 0)
					{
						shif = 0;
						if (sub(abs_s(ps2a), abs_s(ps2), top) > 0)
						{
							ps2  = ps2a;
							shif = 1;
						}
						
						ps2_32 = Load_sh(ps2, (Word16)15, top);
						...
						for (ii3 = 3; ii3 < 32; ii3 += 4)
						{
							...
							L_tmp = sub_sh(ps2_32, dn[2*ii3 + shif], (Word16)15, top);
							...
							L_tmp = add_sh(alp2_32, rr[ii3][ii3], (Word16)12, top);
							L_tmp = add_sh(L_tmp, rr[ii1][ii3], (Word16)13, top);
							L_tmp = L_msu0(L_tmp, (Word16)Q13_gain_i0, rr[ii0][ii3], top);
							L_tmp = sub_sh(L_tmp, rr[ii2][ii3], (Word16)13, top);
							...
							*/
							...
							ps3c = mult(ps3, ps3, top);
							L_tmp  = L_mult(ps3c, alpha, top);
							if (L_msu(L_tmp, psc, alp3, top) > 0)
							{
								ps    = ps3;
								psc   = ps3c;
								alpha = alp3;
						...
						time = sub(time, (Word16)3, top);	/* See note on complexity */
						if (time <= 0)
							goto fin;		/* Maximum time finish    */
					} /* end if >seuil2 */
				} /* end of for ii2 = */
				
				time = sub(time, (Word16)4, top);		/* See note on complexity */
				if (time <= 0)
					goto fin;		/* Maximum time finish    */
			} /* end if >seuil1 */
   } /* end of for ii1 = */
...
fin:
	...
	if (ps >= 0)
	{
		*sign = 0;
		for (i = 0; i < lcode; i++)
		{
			...
			L_tmp  = sub_sh(L_tmp, p1[i], (Word16)11, top);
			L_tmp  = add_sh(L_tmp, p2[i], (Word16)11, top);
			L_tmp  = sub_sh(L_tmp, p3[i], (Word16)11, top);
			cod[i] = store_hi(L_tmp, (Word16)5, top);
		}
	}
	else
	{
	...
	if (ps >= 0)
	{
		for (i = 0; i < lcode; i++)
		{
			...
			L_tmp = add_sh(L_tmp, p2[i], (Word16)11, top);
			L_tmp = sub_sh(L_tmp, p3[i], (Word16)11, top);
			y[i]  = store_hi(L_tmp, (Word16)5, top);
		}
	}
	else
	{
	...
	*shift_code = shift;
	...
	index = shr(ip0, (Word16)1, top);
	index = add(index, shl(shr(ip1, (Word16)3, top), (Word16)5, top), top);
	index = add(index, shl(shr(ip2, (Word16)3, top), (Word16)8, top), top);
	index = add(index, shl(shr(ip3, (Word16)3, top), (Word16)11, top), top);
...
void D_D4i60(Word16 index, Word16 sign, Word16 shift, Word16 F[], Word16 cod[], tetra_op_t* top)
{
	...
	pos0 = shl((Word16)(index & 31), (Word16)1, top);
	
	pos1 = shr((Word16)(index & 224), (Word16)2, top);
	pos1 = add(pos1, (Word16)2, top);
	
	pos2 = shr((Word16)(index & 1792), (Word16)5, top);
	pos2 = add(pos2, (Word16)4, top);
	
	pos3 = shr((Word16)(index & 14336), (Word16)8, top);
	pos3 = add(pos3, (Word16)6, top);
...
void D_Lsp334(Word16 indice[], Word16 lsp[], Word16 lsp_old[], tetra_op_t* top)
{
	Word16 i, temp;
	const Word16* p;
	...
	temp = 917;			/* 917 = 0.028 in Q15 = 50hz around 1000hz */
	temp = sub(temp, lsp[2], top);
	temp = add(temp, lsp[3], top);	/* temp = 0.028 - (lsp[2] - lsp[3])      */
	if (temp > 0)
	{
		temp = shr(temp, (Word16)1, top);
		lsp[2] = add(lsp[2], temp, top);
		lsp[3] = sub(lsp[3], temp, top);
	}
	...
	temp = 1245;			/* 1245= 0.038 in Q15 = 50hz around 1600hz */
	temp = sub(temp, lsp[5], top);
	temp = add(temp, lsp[6], top);	/* temp = 0.038 - (lsp[5] - lsp[6])      */
	if (temp > 0)
	{
		temp = shr(temp, (Word16)1, top);
		lsp[5] = add(lsp[5], temp, top);
		lsp[6] = sub(lsp[6], temp, top);
	}
                ...
                Word16 *gain_pit, Word16 *gain_cod, tetra_op_t* top)
{
	...
	*-------------------------------*/
	...
	L_tmp = L_mac0(L_tmp, *gain_pit, *gain_pit, top);
	
	Log2(L_tmp, &exp, &frac, top);
	
	L_tmp = Load_sh16(exp, top);
	L_tmp = add_sh(L_tmp, frac, (Word16)1, top);	/* Log2(gain_pit**2)    in Q16   */
	L_tmp = sub_sh16(L_tmp, (Word16)24, top);	/* -24.0->gain_pit**2 is in Q24*/
	...
	ener_pit = add(ener_pit, ener_plt, top);
	...
	*-------------------------------*/
	...
	L_tmp = 1;					/* Avoid Log2(0) 			   */
	L_tmp = L_mac0(L_tmp, *gain_cod, *gain_cod, top);
	
	Log2(L_tmp, &exp, &frac, top);
	
	L_tmp = Load_sh16(exp, top);
	L_tmp = add_sh(L_tmp, frac, (Word16)1, top);	/* Log2(gain_code**2) in Q16	   */
	L_tmp = L_shr(L_tmp, (Word16)8, top);		/* Convert from Q16 to Q8        */
	ener_cod = extract_l(L_tmp);
	ener_cod = add(ener_cod, ener_c, top);
	...
	*-----------------------------------------------------------------*/
	...
	if (L_tmp < 0)	
		L_tmp = 0;
	pred_pit = store_hi(L_tmp, (Word16)7, top);	/* result in Q8 			   */
	err_pit  = sub(ener_pit, pred_pit, top);
	...
	*-----------------------------------------------------------------*/
	...
	L_tmp = Load_sh(top->last_ener_cod, (Word16)8, top); /* .5 last_ener_cod  in Q9    */
	L_tmp = add_sh(L_tmp, top->last_ener_pit, (Word16)7, top); /*+.25 last_ener_pit
	in Q9    */
	L_tmp = sub_sh(L_tmp, (Word16)768, (Word16)9, top); /* -3.0 in Q9             */
	if (L_tmp < 0)
		L_tmp = 0;
	pred_cod = store_hi(L_tmp, (Word16)7, top);	/* result in Q8 			   */
	err_cod  = sub(ener_cod, pred_cod, top);
	...
	*-----------------------------------------------------------------*/
	...
	{
		tmp  = sub(*p++, err_pit, top);
		dist = L_mult0(tmp, tmp);
		tmp  = sub(*p++, err_cod, top);
		dist = L_mac0(dist, tmp, tmp, top);
		
		
		
		if (L_sub(dist, dist_min, top) < 0)
		{
			dist_min = dist;
			index = i; 
		}
	...
	*---------------------------------------------------*/
	...
	L_extract(L_tmp, &exp, &frac, top);
	L_tmp = pow2(exp, frac, top);
	if (L_sub(L_tmp, (Word32)4915, top) > 0)
		L_tmp = 4915; /* 4915 = 1.2 in Q12  */
	*gain_pit = extract_l(L_tmp);
...
Word16 G_Code(Word16 xn2[], Word16 y2[], Word16 L_subfr, tetra_op_t* top)
{
	...
	for (i = 0; i < L_subfr; i++)
		s = L_mac0(s, xn2[i], y2[i], top);
	
	exp_xy = norm_l(s);
	xy     = extract_h(L_shl(s, exp_xy, top));
	
	/* Compute scalar product <y2[], y2[]> */
	
	s = 1;					/* Avoid case of all zeros */
	for (i = 0; i < L_subfr; i++)
		s = L_mac0(s, y2[i], y2[i], top);
	
	exp_yy = norm_l(s);
	yy     = extract_h(L_shl(s, exp_yy, top));
	...
	if (top->Overflow == 1)
	{
		...
		for (i = 0; i < L_subfr; i++)
		{
			...
			L_tmp = L_shr(L_tmp, (Word16)6, top);
			s = L_add(s, L_tmp, top);
		}
	...
	xy = shr(xy, (Word16)1, top);			/* Be sure xy < yy */
	gain = div_s(xy, yy, top);
	
	i = add(exp_xy, (Word16)2, top);		/* Denormalization of division */
	i = sub(i, exp_yy, top);
...
Word32 Inter8_M1_3(Word16 x[], tetra_op_t* top)
{
	Word16 i;
	Word32 s;
	static const Word16 coef[8] = 
	{	
...
Word32 Inter8_1_3(Word16 x[], tetra_op_t* top)
{
	Word16 i;
	Word32 s;
	static const Word16 coef[8] = 
...
Word16 Inter32_M1_3(Word16 x[], tetra_op_t* top)
{
	Word16 i;
	Word32 s;
...
Word16 Inter32_1_3(Word16 x[], tetra_op_t* top)
{
	Word16 i;
	Word32 s;
			   ...
			   tetra_op_t* top)
{
	...
	for (i = lag_max; i >= lag_min; i--)
	{
		...
		for (j = 0; j < L_frame; j += 2, p++, p1 += 2)
			t0 = L_mac0(t0, *p, *p1, top);
		
		if (L_sub(t0, max, top) >= 0)
		{
			max    = t0;
			p_max = i;
		}
	...
	max = L_shr(max, (Word16)1, top);	/* for special double precision format */
	L_extract(max, &max_h, &max_l, top);
	...
	for (i = 0; i < L_frame; i += 2, p += 2)
		t0 = L_mac0(t0, *p, *p, top);
	
	/* 1/sqrt(energie),    result in Q30 */
	
	t0 = inv_sqrt(t0, top);
	L_extract(t0, &ener_h, &ener_l, top);
	
	/* max = max/sqrt(energie)                  */
	/* This result will always be on 16 bits !! */
	
	t0 = mpy_32(max_h, max_l, ener_h, ener_l, top);
...
void Norm_Corr(Word16 exc[], Word16 xn[], Word16 h[], Word16 L_subfr,
               Word16 t_min, Word16 t_max, Word16 corr_norm[], tetra_op_t* top)
{
	Word16 i, j, k;
	...
	Convolve(&exc[k], h, excf, L_subfr, top);
	...
	for (i = t_min; i <= t_max; ++i)
	{
		...
		s = 0;
		for (j = 0; j < L_subfr; j++)
			s = L_mac0(s, xn[j], excf[j], top);
		
		s = L_shr(s, (Word16)1, top);		/* Special double precision format */
		L_extract(s, &corr_h, &corr_l, top);
		...
		s = 0;
		for (j = 0; j < L_subfr; j++)
			s = L_mac0(s, excf[j], excf[j], top);
		
		s = inv_sqrt(s, top);			/* Result in Q30 */
		L_extract(s, &norm_hi, &norm_lo, top);
		
		
		/* Normalize correlation = correlation * (1/sqrt(energie)) */
		
		s = mpy_32(corr_h, corr_l, norm_hi, norm_lo, top);
		...
		if (sub(i, t_max, top) != 0)
		{
			k--;
			for (j = L_subfr - 1; j > 0; j--)
			{
				s = L_mult0(exc[k], h[j]);
				s = add_sh(s, excf[j - 1], (Word16)12, top);
				excf[j] = store_hi(s, (Word16)4, top);
			}
			excf[0] = exc[k];
		}
	}
                ...
                Word16 *pit_frac, tetra_op_t* top)
{
	...
	t_min = sub(t0_min, (Word16)Lg_inter, top);
	t_max = add(t0_max, (Word16)Lg_inter, top);
	
	corr = &corr_v[-t_min];
	
	/* Compute normalized correlation between target and filtered excitation */
	
	Norm_Corr(exc, xn, h, L_subfr, t_min, t_max, corr, top);
	...
	if ((i_subfr == 0) && (sub(lag, (Word16)84, top) > 0))
	{
		*pit_frac = 0;
		return (lag);
	}
	...
	frac  = 0;
	L_max = Load_sh(max, (Word16)15, top);
	
	/* test +1/3 */
	
	corr_int = Inter8_1_3(&corr[lag], top);
	if (L_sub(corr_int, L_max, top) >= 0)
    {
		L_max = corr_int;
		frac= 1;
	}
	...
	corr_int = Inter8_M1_3(&corr[lag + 1], top);
	if (L_sub(corr_int, L_max, top) >= 0)
    {
		L_max = corr_int;
		frac= 2;
	...
	corr_int = Inter8_1_3(&corr[lag - 1], top);
	if (L_sub(corr_int, L_max, top) >= 0)
    {
		L_max = corr_int;
		frac= -2;
	...
	corr_int = Inter8_M1_3(&corr[lag], top);
	if (L_sub(corr_int, L_max, top) >= 0)
    {
		L_max = corr_int;
		frac= -1;
	...
	if (sub(frac, (Word16)2, top) == 0)
    {
		frac = -1;
		lag = add(lag, (Word16)1, top);
	};
	if (sub(frac, (Word16) - 2, top) == 0)
    {
		frac = 1;
		lag = sub(lag, (Word16)1, top);
	};
	*pit_frac = frac;
	return (lag);
}
...
Word16 Pitch_Ol_Dec(Word16 signal[], Word16 L_frame, tetra_op_t* top)
{
	...
	*--------------------------------------------------------*/
	...
	for (i= -pit_max; i < L_frame; i += 2)
		t0 = L_mac0(t0, signal[i], signal[i], top);
	...
	*--------------------------------------------------------*/
	...
	if (top->Overflow == 1)
	{
		for (i = 0, j = 0; i < L_frame; i += 2, j++)
			sig_dec[j] = shr(signal[i], (Word16)6, top);
	}
	else if (L_sub(t0, L_shl((Word32)1, (Word16)22, top), top) < 0)
	{
		for (i = 0, j = 0; i < L_frame; i += 2, j++)
			sig_dec[j] = shl(signal[i], (Word16)4, top);
	}
	else
	{
		for (i = 0, j = 0; i < L_frame; i += 2, j++)
	...
	*--------------------------------------------------------------------*/
	...
	if (sub(mult((Word16)max1, (Word16)seuil, top), (Word16)max2, top)  < 0)
    {
		max1 = max2;
		p_max1 = p_max2;
	}
	
	if (sub(mult((Word16)max1, (Word16)seuil, top), (Word16)max3, top)  < 0)
		p_max1 = p_max3;
...
void Post_Process(Word16 signal[], Word16 lg, tetra_op_t* top)
{
	...
	for (i = 0; i < lg; i++)
		signal[i] = add(signal[i], signal[i], top);
}
...
void Pred_Lt(Word16 exc[], Word16 T0, Word16 frac, Word16 L_subfr, tetra_op_t* top)
{
	...
	else if (sub(frac, (Word16)1, top) == 0)
	{
		for (i = 0; i < L_subfr; i++)
			exc[i] = Inter32_1_3(&exc[i - T0], top);
	}
	else if (sub(frac, (Word16) - 1, top) == 0)
	{
		for (i = 0; i < L_subfr; i++)
			exc[i] = Inter32_M1_3(&exc[i - T0], top);
	}
...
void Pre_Process(Word16 signal[], Word16 lg, tetra_op_t* top)
{
	...
	for (i = 0; i < lg; i++)
	{
		...
		L_tmp     = Load_sh(top->x0, (Word16)15, top);
		L_tmp     = sub_sh(L_tmp, x1, (Word16)15, top);
		L_tmp     = L_mac(L_tmp, top->y_hi, (Word16)32735, top);
		L_tmp     = add_sh(L_tmp, mult(top->y_lo, (Word16)32735, top), (Word16)1, top);
		signal[i] = extract_h(L_tmp);
		top->y_hi = extract_h(L_tmp);
		top->y_lo = extract_l(sub_sh(L_shr(L_tmp, (Word16)1, top), top->y_hi, (Word16)15, top));
	}
	return;
}
...
void  Prm2bits_Tetra_8(Word16 prm[], unsigned char bits[], tetra_op_t* top)
{
	...
	for (i = 0; i < PRM_NO; i++)
	{
		int2bin_8(prm[i], bitno[i], bits, top);
		bits += bitno[i];
	}
	return;
}
